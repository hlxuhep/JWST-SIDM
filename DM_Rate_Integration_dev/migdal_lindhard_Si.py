...
# Silicon parameters
N_q   = 800
N_E   = 2000                 # 0.05 eV steps up to 100 eV
dE    = 0.05 * nu.eV
dq    = 0.01 * nu.aEM * nu.mElectron
E_max = N_E * dE
q_max = N_q * dq
energy_gap = 1.11 * nu.eV    # Si band gap
epsilon    = 3.6 * nu.eV     # mean energy per e-h pair (kept for Q-related routines)
# For rates per detector mass, use per-nucleus mass so N_T = 1/M_cell
M_cell     = 28.0855 * nu.AMU
...
a_c   = 5.431 * nu.Angstrom  # Si lattice constant
n_val = 32
ne_c  = n_val / a_c**3
...
mA = 1e30 * nu.eV  # contact/heavy mediator limit => F_DM ≈ 1
...
def dRdEe_halo(Ee, sigma_n, mDM):
    ...
    # Valence ionic charges for Si (Z_ion)
    Z_ion = {"si": 4.0}
...
# --- Plotting: arXiv:1908.10881 Fig.2 (top-right) style: Silicon, contact (F_DM=1) ---
def plot_1908_fig2_silicon_contact_lindhard(outname="fig2_1908_Si_contact_lindhard.png"):
    """Plot dR/dln(ΔE) vs ΔE for Si Migdal (contact interaction), averaged in 1 eV bins.

    Implements the note in 1908.10881: "The rates are averaged over a 1 eV energy bin."
    <dR/dEe>_bin = (1/1eV) * ∫_{bin} dEe dR/dEe, and dR/dlnΔE = ΔE_center * <dR/dEe>_bin.
    y-axis units: 1/(kg*yr).
    """
    sigma_fig = 1e-38 * nu.cm**2
    masses = [0.1 * nu.GeV, 1.0 * nu.GeV]  # 100 MeV, 1 GeV

    binw = 1.0 * nu.eV
    Emax_plot = E_grid[-1]
    edges = np.arange(0.0, float(Emax_plot / nu.eV) + 1e-9, 1.0) * nu.eV
    centers = 0.5 * (edges[:-1] + edges[1:])

    fig, ax = plt.subplots()
    for mDM in masses:
        # compute dR/dEe on internal E_grid
        dRdEe_vals = np.array([dRdEe_halo(Ei, sigma_fig, mDM) for Ei in E_grid], dtype=float)

        dln_vals = np.zeros_like(centers, dtype=float)
        for j in range(len(centers)):
            lo, hi = edges[j], edges[j + 1]
            mask = (E_grid >= lo) & (E_grid < hi)
            if not np.any(mask):
                continue
            avg = np.sum(dRdEe_vals[mask]) * dE / binw
            dln_vals[j] = float((centers[j] * avg) * (nu.kg * nu.year))

        ax.plot(centers / nu.eV, dln_vals, label=rf"$m_\chi={mDM/nu.MeV:.0f}\ \mathrm{{MeV}}$")

    ax.set_yscale('log')
    ax.set_xlim(0, 60)
    ax.set_ylim(1e-4, 1e3)
    ax.set_xlabel(r"$\Delta E\ \mathrm{[eV]}$")
    ax.set_ylabel(r"$\mathrm{d}R/\mathrm{d}\ln\Delta E\ \mathrm{[1/(kg\,yr)]}$")
    ax.text(0.62, 0.92, r"$\bar\sigma_n=10^{-38}\ \mathrm{cm}^2$", transform=ax.transAxes)
    ax.text(0.62, 0.84, r"$F_{\rm DM}=1$", transform=ax.transAxes)
...
# 10 per kg per year with cs 1e-38
...
if __name__ == "__main__":
    plot_1908_fig2_silicon_contact_lindhard()